...
python -m src.models.train         # trains LightGBM, logs to MLflow, saves model + schema
python -m src.models.evaluate      # cost-based threshold analysis
python -m src.models.cost_sensitivity   # threshold sensitivity across cost assumptions
python -m src.models.explain       # SHAP plots, saved to models/explainability/
...
Test it:
curl -X POST http://127.0.0.1:8000/predict -H "Content-Type: application/json" -d "{"TransactionAmt": 150.0, "TransactionDT": 86400, "card1": 13926}"

The API loads a saved card-level reference table (`models/card_reference.parquet`)
and the exact training feature schema (`models/feature_schema.json`) at startup,
so it can build a full model-compatible row from a partial incoming request,
missing fields become `NaN`, which LightGBM handles natively, rather than
crashing on a shape mismatch against the 439 features the model was trained on.

## Known limitation, documented rather than hidden

Prediction quality depends heavily on how complete the incoming transaction
record is. A request with only 3-4 fields will have ~430+ features as `NaN`.
A real production deployment would receive the full transaction payload from
a payment processor, this API's schema-matching logic handles that gracefully,
but was only tested here with partial requests for demonstration.

## Project structure

- `src/data/` — loading and merging raw tables
- `src/features/` — feature engineering, shared between training and serving
- `src/models/` — training, evaluation, cost sensitivity, and SHAP explainability
- `src/api/` — FastAPI serving layer
- `notebooks/` — exploratory work only, nothing here should be load-bearing
- `tests/` — unit tests for feature functions and threshold/cost logic